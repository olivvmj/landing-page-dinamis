                            ...
                            <form id="form" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <div class="mb-3">
                                                <label for="fitur" class="form-label">fitur Aplikasi</label>
                                                <input type="text" id="fitur" class="form-control" name="fitur"
                                                    value="{{ old('fitur') }}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" id="desk_fitur" class="form-control" name="desk_fitur"
                                                    value="{{ old('desk_fitur') }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
@section('component_js')
    ...
    <script>
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if (imagePath) {
                                imageUrl = "{{ url('storage/landingpage/fitur/') }}/" + imagePath;
                            } else {
        ...
        function create() {
            ...
            $('.modal-title').text('Tambah Data Fitur Parkirkan');
            // $('#btnSave').on('click', function() {
            //     submit();
            // })
        }
        ...
        function edit(id) {
            ...
            $.get(url, function(response) {
                ...
                $('#modal_form').modal('show');
                $('.modal-title').text('Edit Data Fitur Parkirkan');
                // $('.dropify').dropify();
                ...
                // var imageName = data.fitur.image;

                df = df.data('dropify');
                df.resetPreview();
                df.clearElement();
                df.settings.defaultFile = `{{ asset('storage/landingpage/fitur') }}/`+data.image;
                df.destroy();
                df.init();
        ...
        function submit() {
            ...
            // var submit_method = '';
            // console.log(judul);            
            var url = "{{ route('kelola-fitur.store') }}";
            ...
            if (submit_method == 'edit') {
                ...
                // var url = "{{ route('kelola-fitur.store') }}";
                var url = "{{ route('kelola-fitur.update', ':id') }}";
                url = url.replace(':id', id);
